...
const Experience = () => {
  ...
  return (
    ...
    >
      ...
      <img className="exp-accent exp-accent--1" src="/assets/premium/fig1.png" alt="" aria-hidden="true" />
      <img className="exp-accent exp-accent--2" src="/assets/premium/fig2.png" alt="" aria-hidden="true" />

      <div className="container exp-inner">
        <header className="exp-head">
          <p className="eyebrow reveal reveal-up">{sectionCopy.experience.eyebrow}</p>
          <h2 className="section-title reveal reveal-up">{sectionCopy.experience.title}</h2>
        </header>
        ...
        <div className="exp-columns">
          ...
          <div className="exp-col reveal reveal-up">
            ...
            <div className="exp-timeline">
              {workItems.map((item, i) => (
                <Card item={item} index={i} key={`${item.company}-${item.role}`} />
              ))}
            </div>
          </div>
          ...
          <div className="exp-col reveal reveal-up" style={{ '--reveal-i': 1 }}>
            ...
            <div className="exp-timeline">
              {eduItems.map((item, i) => (
                <Card item={item} index={i + workItems.length} key={`${item.company}-${item.role}`} />
              ))}
            </div>
          </div>
        </div>
